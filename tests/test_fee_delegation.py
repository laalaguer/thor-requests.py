...
def test_vet_transfer(solo_connector, solo_wallet, clean_wallet):
    ...
    res = solo_connector.transfer_vet(clean_wallet, sender, 3 * (10 ** 18), solo_wallet)
    assert res["id"]
    tx_id = res["id"]
    receipt = solo_connector.wait_for_tx_receipt(tx_id)
...
def test_transfer_vtho_easy(
    ...
    res = solo_connector.transfer_vtho(clean_wallet, solo_wallet.getAddress(), 3 * (10 ** 18), solo_wallet)
    tx_id = res["id"]
    receipt = solo_connector.wait_for_tx_receipt(tx_id)
